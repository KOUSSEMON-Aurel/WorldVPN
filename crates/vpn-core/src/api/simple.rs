...
pub fn set_logged_user(username: String) {
    if let Ok(mut guard) = LOGGED_USER.lock() {
        *guard = Some(username);
    }
}

pub fn set_private_key(key: Vec<u8>) {
    if let Ok(mut guard) = PRIVATE_KEY.lock() {
        *guard = Some(key);
    }
}
...
pub async fn login_anonymously_async(private_key_bytes: Vec<u8>) -> anyhow::Result<String> {
...
pub fn login_anonymously(private_key_bytes: Vec<u8>) -> anyhow::Result<String> {
    if let Ok(handle) = tokio::runtime::Handle::try_current() {
        handle.block_on(login_anonymously_async(private_key_bytes))
    } else {
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(login_anonymously_async(private_key_bytes))
    }
}

pub async fn start_vpn_matchmaking_async(protocol_str: String, country_code: String, node_group: String) -> anyhow::Result<String> {
...
pub fn start_vpn_matchmaking(protocol_str: String, country_code: String, node_group: String) -> anyhow::Result<String> {
    if let Ok(handle) = tokio::runtime::Handle::try_current() {
        handle.block_on(start_vpn_matchmaking_async(protocol_str, country_code, node_group))
    } else {
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(start_vpn_matchmaking_async(protocol_str, country_code, node_group))
    }
}

pub async fn start_sharing_async() -> anyhow::Result<()> {
...
pub fn start_sharing() -> anyhow::Result<()> {
    if let Ok(handle) = tokio::runtime::Handle::try_current() {
        handle.block_on(start_sharing_async())
    } else {
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(start_sharing_async())
    }
}
